...
2. Environment variables: Override specific items from platform config
   ...
   - VLLM_FL_FLAGOS_BLACKLIST: FlagGems operator blacklist
...
# Directory containing this file (config/)
...
def get_flaggems_blacklist(config: Optional[dict] = None) -> Optional[list[str]]:
    """
    Extract FlagGems operator blacklist from config.

    Args:
        config: Configuration dict. If None, load from platform config.

    Returns:
        List of blacklisted FlagGems operator names.
    """
    if config is None:
        config = load_platform_config()
    if config is None:
    ...
    blacklist = config.get('flaggems_blacklist', [])
